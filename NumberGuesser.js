...
// make a variable for the number the Akinator is guessing. The varibale is assigned to a random number
var secretNumber = Math.floor(Math.random()*(maximum - minimum + 1)) + minimum;
//This line will keep the reward hidden until the unil the Guesser makes the correct guess
setProperty("Cont_button", "hidden", true);
//The Text box above will describe the kind of number they are thinking of
setText("Status_screen", " Try to guess the number the Akinator is thinking of ! enter your guess in the
...
if (isDuplicate) {
setText("Status_screen", "You've guessed that number already! Please enter a different number.");
return;
}
previousGuesses.push(UsersGuess);
//establish the outcome: if the user guesses the number to be right, the user wins, if not, the user lo
if (gamestatus === "Win") {
setText("Status_screen", "Congratulations! you guessed the correct number!");
...
var finalGuess = 1;
//Check to see if the number they guessed equals the number the akinator is guessing
...
if (UsersGuess > secretNumber) {
setText("Status_screen", "Nope! The number you guessed was too High! try again!");
} else if ((UsersGuess < secretNumber)) {
setText("Status_screen", "Nope! the number you guessed was too low! Try again!");
}
// This statement will check to see your guess was close or not.
if (Math.abs(UsersGuess - secretNumber) <= 10 && UsersGuess < secretNumber) {
setText("Status_screen", "You're getting close! The number you guessed was too low. keep guessing
} else if (Math.abs(UsersGuess - secretNumber) >= 10 && UsersGuess > secretNumber) {
setText("Status_screen", "You're getting close! The number you guessed was too high. keep guessin
}
if (Math.abs(UsersGuess - secretNumber) <= 5 && UsersGuess < secretNumber) {
setText("Status_screen", "You're getting even closer! The number you guessed was too low. keep gu
} else if (Math.abs(UsersGuess - secretNumber) >= 5 && UsersGuess > secretNumber) {
setText("Status_screen", "You're getting even closer! The number you guessed was too high. keep g
}
...
setText("Status_screen", "Uh oh! your down to your final guess! use it wisely....");
}
}
}
...
//Have an image temperature display based on how close the users guess is
